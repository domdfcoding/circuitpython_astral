...
class Location:
	...
	def __init__(self, info: Optional[LocationInfo] = None):
		...
		See the :attr:`timezone` property for a method of obtaining time zone names
		"""
	...
	def sun(
		...
		:param observer_elevation: Elevation of the observer in metres above the location.
	...
	def dawn(
		...
		By default this is 6 degrees but can be changed by setting the :attr:`Astral.solar_depression` property.
	...
	def sunset(
		...
		Sunset time is the time in the evening when the sun is a 0.833 degrees below the horizon. This is to account for refraction.
	...
	def dusk(
		...
		By default this is 6 degrees but can be changed by setting the :attr:`solar_depression` property.)
	...
	def rahukaalam(
		...
		:param date: The date for which to calculate the rahukaalam period.
			A value of :py:obj:`None` uses the current date.
